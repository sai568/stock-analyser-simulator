...
def Sample_strategy(start,end,investment,n_days_for_measuring_performance,top_n_stocks):
    ...
    top_n = performance.sort_values(ascending = False).iloc[0:(int(top_n_stocks)-1)].reset_index()
...
if(st.button('Submit')):
    ...
    st.success('Wait, while we fetch data fro you.')
    ...
    Results = pd.DataFrame({'Benchmark_strategy': Voltality(Benchmark_EC),'Performance_based_strategy': Voltality(Sample_EC),'Nifty_50':Voltality(nifty_50)}).T
